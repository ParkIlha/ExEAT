...
> 
> 진입 타이밍이 아닌 **EXIT 타이밍**을 알려주는 외식업 사장용 진단 도구.  
> 대만카스테라·탕후루·두바이초콜릿... 트렌드 메뉴를 따라 들였다가 재고만 남긴 경험, 이제 데이터로 막습니다.

카페·디저트·식당·푸드트럭 등 외식업 전반에 적용됩니다.
...
| **F2 GO/WAIT/STOP 판정** | AI + 수명주기 알고리즘 통합 판정 |
| **F3 트렌드 그래프** | 12주 실측 + 4주 예측선 + 변곡점 마커 + 쇼핑클릭 이중 라인 |
| **F4 지역 적합도** | 행정안전부 인구 데이터 기반 지역별 도입 적합도 |
| **F5 위험도 종합 점수** | 모멘텀·변동성·정점하락률·변화량 4지표 → 0~100 위험도 |
| **F6 손익분기 시뮬레이터** | 재료비·판매가·판매량 → EXIT 전 예상 수익 계산 |
| **F7 액션 플랜** | 즉시(1주)·단기(1개월)·중기(3개월) 단계별 구체 행동 플랜 |
| **F8 최악의 시나리오** | Worst Case 매출 영향 + 대안 메뉴 제시 |
