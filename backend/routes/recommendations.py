...
KEYWORD_POOL = [
    # 음료·카페
    "흑임자라떼", "말차라떼", "우베라떼", "복숭아아이스티", "얼그레이라떼",
    # 디저트
    "크로플", "마카롱", "소금빵", "휘낭시에", "바스크치즈케이크",
    # 푸드
    "마라탕", "마라샹궈", "로제파스타", "크림파스타", "한우타코야키",
    # 스낵·길거리
    "붕어빵", "떡볶이", "핫도그", "타코야키",
    # 주류·안주
    "하이볼", "하이볼안주", "이자카야",
...
def _score(item: dict) -> float:
    ...
    cycle_bonus = {
        "RISING": 20, "EMERGING": 15, "GROWING": 10,
    }.get(cycle, 0)
    return base + cycle_bonus + conf * 20 - risk * 0.3
...
def get_recommendations():
    ...
    if not ready:
        # 아직 분석 중 — 빈 배열 반환 (프론트에서 로딩 처리)
        return jsonify({"items": [], "ready": False})

    # GO 메뉴 우선, 없으면 WAIT까지 포함해 5개 채우기
