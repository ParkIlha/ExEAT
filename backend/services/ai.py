...
- 모든 필드는 한국어
- **dataInsight**: 수치를 직접 나열하지 말고, "지금 이 메뉴는 ~한 상태예요" 식으로 해석해서 설명. 4~6줄 분량으로 현재 상태·흐름·주의사항을 충분히 서술
- **marketContext**: "이 시장은 ~해서 ~을 조심해야 해요" 식의 조언형. 3~5줄로 시장 환경·경쟁·소비자 변화를 분석하고 실천 조언으로 마무리
...
출력 JSON 스키마:
{
  ...
  "dataInsight": "지금 이 메뉴 상태를 쉽게 해석한 4~6줄. 수치 직접 나열 금지, 의미 위주. 현재 상태 → 최근 변화 흐름 → 사장님이 주의할 점 순서로 서술.",
  "marketContext": "이 시장이나 경쟁 상황을 3~5줄로 서술. 경쟁 강도·소비자 반응 변화·시장 포화도 등 맥락 설명 후 구체적 조언으로 마무리.",
